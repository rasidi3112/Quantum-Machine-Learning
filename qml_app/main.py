...
def train(
    ...
    if model.lower() == "vqc":
        ...
        model_state = {
            ...
            "c_regularization": cfg.model.kernel_regularization,
            "use_complex_device": cfg.model.use_complex_device,
            "trained_params": getattr(qsvc, "trained_params_", None),
        }
        joblib.dump(model_state, model_path)
        print(f"Parameter kernel tersimpan di {model_path}")
...
def evaluate(
    ...
    if model.lower() == "vqc":
        ...
        model_path = weight_path or artifacts_dir / "kernel_model_params.pkl"
        if not model_path.exists():
            raise FileNotFoundError(f"Tidak menemukan parameter kernel di {model_path}")

        model_state = joblib.load(model_path)
        qsvc = QuantumKernelClassifier(
            n_qubits=model_state["n_qubits"],
            feature_layers=model_state["feature_layers"],
            shots=model_state["shots"],
